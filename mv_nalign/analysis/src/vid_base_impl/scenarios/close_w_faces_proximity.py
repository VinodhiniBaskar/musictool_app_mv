...
class WomenCloseProximity:
    ...
    async def get_data(self, type):
        if type == 'img':
            faces = self.face_analyzer.image_run_detection(self.file, self.bucket, callback=None)
        else:
            faces = self.face_analyzer.video_run_detection(self.file, self.bucket, callback=None)
...
def old_new_frame_postprocessing(faces):
    ...
    all_boxes = [face for face in faces]
...
def frame_postprocessing(faces):
    ...
    faces = list(filter(lambda x: x["Gender"]["Value"] == "Female", faces))
    if not len(faces) > 1:
        return {"ViolationFound": False, "CloseBoxes": response_boxes, "Boxes": all_boxes}
    # Sort all faces to make their position in array same as on the image
    faces = sorted(faces, key=lambda x: (x["BoundingBox"]["Left"], x["BoundingBox"]["Width"]))
